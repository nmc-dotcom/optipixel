...
interface SpriteTimelineProps {
  ...
  width: number;
  height: number;
  onSelectFrame: (layerId: string) => void;
  onAddFrame: () => void;
  onDuplicateFrame: (layerId: string) => void;
  onDeleteFrame: (layerId: string) => void;
  onMoveFrame: (layerId: string, direction: 'left' | 'right') => void;
...
export const SpriteTimeline: React.FC<SpriteTimelineProps> = ({
  ...
  const handleTogglePlay = () => {
    if (!isPlaying) {
      if (layers.length <= 1) {
        ...
        onDuplicateFrame(activeLayerId);
      }
      const activeIdx = visibleLayers.findIndex(l => l.id === activeLayerId);
      const startIdx = activeIdx >= 0 ? activeIdx : 0;
      setCurrentPlayIndex(startIdx);
      setPingPongDir(1);
      setIsPlaying(true);

      if (visibleLayers[startIdx]) {
        onSelectFrame(visibleLayers[startIdx].id);
      }
    } else {
      setIsPlaying(false);
    }
  ...
  useEffect(() => {
    if (!isPlaying || visibleLayers.length <= 1) return;
    ...
    const interval = setInterval(() => {
      setCurrentPlayIndex(prev => {
        ...
        if (loopMode === 'loop') {
          // 🔁 무한 순환 루프: 0 -> 1 -> 2 -> ... -> 0 -> 1
          next = (prev + 1) % visibleLayers.length;
        } else if (loopMode === 'pingpong') {
          // 🔀 왕복 핑퐁 루프: 0 -> 1 -> 2 -> 1 -> 0 -> 1
          let dir = pingPongDir;
          if (prev + dir >= visibleLayers.length) {
            dir = -1;
            setPingPongDir(-1);
          } else if (prev + dir < 0) {
            dir = 1;
          ...
          next = Math.max(0, Math.min(visibleLayers.length - 1, prev + dir));
        } else if (loopMode === 'once') {
          // 🔂 1회 재생 후 자동 정지
          if (prev + 1 >= visibleLayers.length) {
            setIsPlaying(false);
            return prev;
          }
          next = prev + 1;
        ...
        if (visibleLayers[next]) {
          onSelectFrame(visibleLayers[next].id);
        }
  ...
  }, [isPlaying, fps, visibleLayers, loopMode, pingPongDir, onSelectFrame]);
  ...
  useEffect(() => {
    ...
    const target = isPlaying 
      ? visibleLayers[currentPlayIndex] 
  ...
  }, [isPlaying, currentPlayIndex, activeLayerId, layers, width, height, visibleLayers]);
      ...
      <div className="flex items-center justify-between px-3 py-1.5 bg-[#161616] border-b border-gray-800/80">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {isPlaying && visibleLayers.length > 1 && (
              <span className="text-[10px] font-mono font-bold px-1.5 py-0.5 rounded bg-emerald-900/40 border border-emerald-700/50 text-emerald-300 animate-pulse">
                {currentPlayIndex + 1}/{visibleLayers.length}
              </span>
            )}
        ...
        <div className="flex items-center gap-1.5">
          <button
            onClick={() => onDuplicateFrame(activeLayerId)}
        ...
        <div className="flex items-center gap-3 p-2.5 overflow-x-auto bg-[#0A0A0A]/90 min-h-[76px]">
          ...
          <div className="flex items-center gap-2 flex-1">
            {layers.map((layer, index) => {
              ...
              const thumbUrl = renderThumbnail(layer.pixels);
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[8px] font-mono text-gray-400 truncate max-w-full">
                    {layer.name.length > 7 ? `${layer.name.slice(0, 6)}..` : layer.name}
                  </span>
                </div>
              );
            })}
