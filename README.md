...
- **이미지 → 도트 변환**: 사진/이미지를 업로드해 지정한 해상도와 색상 수로
  ...
  크기를 맞출 수 있습니다.
- **프레임 & 레이어**: 애니메이션 프레임 하나가 자기만의 레이어 스택을 가집니다.
  한 프레임 안에 배경·캐릭터·효과를 따로 두고, 프레임은 타임라인에서 추가·복제·
  순서 변경할 수 있습니다.
- **레이어 & 그룹**: 여러 레이어를 그룹으로 묶어 관리하고, 표시/숨김, 잠금,
  불투명도, 병합, 순서 이동을 지원합니다.
- **커스텀 팔레트**: 레트로/모던 프리셋 팔레트를 쓰거나 직접 만든 팔레트를
  브라우저에 저장해 재사용할 수 있습니다.
- **애니메이션 프리뷰**: 루프 / 핑퐁 / 1회 재생 모드로 애니메이션을 실시간
  미리볼 수 있습니다. 어니언 스킨은 이전 프레임을 합성해 잔상으로 보여줍니다.
...
프로젝트 파일은 `version: 2`이며 프레임이 레이어를 소유합니다. 레이어 하나가 곧
프레임 하나였던 `version: 1` 파일은 불러올 때 자동 변환되는데, 그 시절 레이어는
캔버스에 함께 합성되어 보이던 것이므로 프레임으로 흩지 않고 **한 프레임 안에
그대로 쌓아** 화면을 보존합니다. 애니메이션 용도로 쓰던 파일이라면 타임라인에서
레이어를 프레임으로 나눠주면 됩니다.
